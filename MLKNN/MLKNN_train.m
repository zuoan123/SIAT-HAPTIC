...
%MLKNN_train 训练一个基于k近邻算法的multi_labels分类器
%
%    Syntax
%
...
%
%       KNNML_train 输入,
%           train_data   - 一个 MxN 的数组, 训练实例中的第i个实例被存储在数组的 train_data(i,:)中
%           train_target - 一个QxM 的数组, if the ith training instance belongs to the jth class, then train_target(j,i) equals +1, otherwise train_target(j,i) equals -1
...
%计算训练实例间的距离
    ...
    str=version('-release');  %返回MATLAB当前运行的发行版编号
